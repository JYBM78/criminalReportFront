...
private mostrarErrorDatos(mensaje: string, redirigirLogin: boolean = false) {
 ...
 public actualizarCuenta() {
  ...
  this.RegistroService.actualizarUsuario(
    ...
    next: (data) => {
      console.log("Respuesta de actualización:", JSON.stringify(data));
    ...
    error: (error) => {
      ...
      if (error.status === 500) {
        Swal.fire({
          text: error.error.data || 'Error interno del servidor', 
          icon: 'error',
          showConfirmButton: true
        });
      } else if (error.status === 400) {
        Swal.fire({
          text: error.error.mensaje || 'Datos inválidos', 
          icon: 'warning',
          showConfirmButton: true
        });
      } else {
